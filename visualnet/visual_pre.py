...
class TrackerSiamFC(Tracker):
    ...
    def init(self, examplar, box):
        ...
        box = np.array([
            box[1] + box[3] / 2,
            box[0] + box[2] / 2,
            box[3], box[2]], dtype=np.float32)  # 将输入的模板区域坐标转换为中心点坐标、高度和宽度
        self.target_sz = box[2:]
        self.z_sz = np.max(self.target_sz)
        self.scale_factors = np.array([ 0.75, 1, 1.5])  # 计算模板区域的最大尺寸，并定义一个尺度因子数组
        # exemplar features
        z = torch.from_numpy(examplar).to(
            self.device).permute(2, 0, 1).unsqueeze(0).float()
        self.kernel = self.net.backbone(z)  # 将模板图像输入到模型的骨干网络中，以提取特征表示
        self.scale = 1  # 将初始尺度设置为1
    ...
    def update(self, img):
        ...
        self.instance_sz = self.x_sz * self.cfg.exemplar_sz / self.z_sz  # 调整后的输入搜索图像的尺寸
        ...
        for i in range(len(self.scale_factors)):
            ...
            x = torch.from_numpy(x_ins).to(
                self.device).permute(2, 0, 1).unsqueeze(0).float()
            x = self.net.backbone(x)  # 对输入图像进行特征提取

            responses = self.net.head(self.kernel, x)  # 调用了模型的头部部分，并传入了模板（kernel）和特征图作为输入，以计算目标响应
            responses = responses.squeeze(0).squeeze(0).cpu().numpy()
            responses_list.append(responses)

        # peak scale 响应图的处理和最大响应位置的定位
        ...
        # locate target center 定位目标的中心位置，并调整响应图的尺寸
    ...
    def observ(self, examplar, refGT, sample_img):
        ...
        self.init(examplar, refGT)  # 调用 init 方法初始化目标的模板和位置
        box, re_box, scale_id = self.update(sample_img)  # 调用 update 方法更新目标的位置信息
