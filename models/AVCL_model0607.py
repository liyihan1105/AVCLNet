...
class KDNet(nn.Module):
    def __init__(self, net_path_vi=None, net_path_au=None, net_path_au_student=None, use_student_only=False):
        ...
        self.discriminator_vi = VisualDiscriminatorKDnet()
        self.generator_au = AudioGeneratorKDnet(256, 256)
        self.discriminator_au = AudioDiscriminatorKDnet()
    ...
    def forward(self, ref, img0, img1, img2, imgMask0, imgMask1, imgMask2, auFr, auFr_dou):
            ...
            Fvi_student_gen = self.LN1_vi(Fvi_student_gen)
            # 视觉教师模型 net1
            Fvi_teacher1 = self.net1(ref, img0, img1, img2)
            Fvi_teacher1 = self.LN1_vi(Fvi_teacher1)
            # 计算对抗损失（使用二元交叉熵损失）
            discriminator_loss_vi = F.binary_cross_entropy_with_logits(Fvi_teacher1, torch.ones_like(Fvi_teacher1)) + \
                                    F.binary_cross_entropy_with_logits(Fvi_student_gen,
                                                                       torch.zeros_like(Fvi_student_gen))

            # 生成损失（使用均方误差损失）
            generator_loss_vi = F.mse_loss(Fvi_student_gen, Fvi_teacher1)

            Fau_student_gen = self.generator_au(Fau_student)
            Fau_student_gen = self.LN1_au(Fau_student_gen)
            # 听觉教师模型
            Fau_teacher1 = self.net2(auFr).permute(0, 3, 1, 2)
            Fau_teacher1 = self.LN1_au(Fau_teacher1)
            # 计算对抗损失（使用二元交叉熵损失）
            discriminator_loss_au = F.binary_cross_entropy_with_logits(Fau_teacher1, torch.ones_like(Fau_teacher1)) + \
                                    F.binary_cross_entropy_with_logits(Fau_student_gen,
                                                                       torch.zeros_like(Fau_student_gen))

            # 生成损失（使用均方误差损失）
            generator_loss_au = F.mse_loss(Fvi_student_gen, Fvi_teacher1)
            ...
            Fvi_teacher2 = Fvi_teacher1.permute(0, 2, 3, 1).view(b, -1, c)
            Fvi_teacher2 = self.LN_vi(Fvi_teacher2)
            Fau_teacher2 = Fvi_teacher1.permute(0, 2, 3, 1).view(b, -1, c)
            Fau_teacher2 = self.LN_vi(Fau_teacher2)

            # 执行知识蒸馏L1损失
            vision_distillation_loss = F.l1_loss(out_av, Fvi_teacher2)
            audio_distillation_loss = F.l1_loss(out_av, Fau_teacher2)
            total_loss = vision_feature_loss + audio_feature_loss + vision_distillation_loss + audio_distillation_loss + generator_loss_vi + discriminator_loss_vi + generator_loss_au + discriminator_loss_au

            return out_pred, out_evl, total_loss
...
class VisualDiscriminatorKDnet(nn.Module):
    def __init__(self):
        super(VisualDiscriminatorKDnet, self).__init__()
...
class AudioDiscriminatorKDnet(nn.Module):
    def __init__(self):
        super(AudioDiscriminatorKDnet, self).__init__()
        self.main = nn.Sequential(
            nn.Conv2d(256, 64, kernel_size=4, stride=2, padding=1),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv2d(64, 128, kernel_size=4, stride=2, padding=1),
