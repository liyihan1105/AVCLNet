...
class KDNet(nn.Module):
    def __init__(self, net_path_vi=None, net_path_au=None, net_path_au_student=None, use_student_only=False):
        ...
        self.DualAtt = DualFeatureAttentionFusionModule(256)
    ...
    def forward(self, ref, img0, img1, img2, imgMask0, imgMask1, imgMask2, auFr, auFr_dou):
            ...
            Fvi_student_Dual = self.DualAtt(Fvi_student_gen, Fvi_student)
            Fau_student_Dual = self.DualAtt(Fau_student_gen, Fau_student)

            Fvi_student_pe = Fvi_student_Dual + self.PE_vi(Fvi_student)
            Fau_student_pe = Fau_student_Dual + self.PE_au(Fau_student)
            ...
            # feature_output = Fvi_teacher.detach().cpu().numpy()  # 将 tensor 转为 numpy 格式
            #
            # # 如果 feature_output 是二维的，可以直接进行 SVD 和方差计算
            # # 如果是高维 (batch, channel, height, width)，先 reshape 成二维
            # feature_flattened = feature_output.reshape(feature_output.shape[0], -1)
            #
            # # 1. 计算奇异值
            # U, singular_values, Vt = np.linalg.svd(feature_flattened, full_matrices=False)
            #
            # # 2. 计算方差
            # variance = np.var(feature_flattened, axis=0)  # 按列计算方差
            #
            # # 输出结果
...
class AttentionModule(nn.Module):
    def __init__(self, in_channels, reduction=16):
        ...
        self.fc1 = nn.Linear(in_channels, in_channels // reduction, bias=False)  # 全连接层1
        self.relu = nn.ReLU(inplace=True)  # ReLU激活函数
        self.fc2 = nn.Linear(in_channels // reduction, in_channels, bias=False)  # 全连接层2
        self.sigmoid = nn.Sigmoid()  # Sigmoid激活函数

    def forward(self, x):
        b, c, _, _ = x.size()  # 获取输入的batch size和通道数
        y = self.global_avg_pool(x).view(b, c)  # 全局平均池化并展平
        y = self.fc1(y)  # 全连接层1
        y = self.relu(y)  # ReLU激活
        y = self.fc2(y)  # 全连接层2
        y = self.sigmoid(y).view(b, c, 1, 1)  # Sigmoid激活并调整维度
        return x * y.expand_as(x)  # 加权

class DualFeatureAttentionFusionModule(nn.Module):
    def __init__(self, in_channels, reduction=16):
        super(DualFeatureAttentionFusionModule, self).__init__()
        self.attention = AttentionModule(in_channels * 2, reduction)  # 注意力模块

    def forward(self, f1, f2):
        fused = torch.cat([f1, f2], dim=1)  # 特征图拼接
        attention_fused = self.attention(fused)  # 应用注意力机制
        f1_weighted, f2_weighted = torch.chunk(attention_fused, 2, dim=1)  # 分离特征图
        return f1_weighted + f2_weighted  # 融合
