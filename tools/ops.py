...
def getSE(sequence,cam_number):#1-index
    if sequence == 'seq01-1p-0000':
        startFRlist = [109, 103, 74]# 109
        endFRlist = [5314, 5308, 5279]# 5314
...
def splitDataset(datasetPath, seqList,
                 splitType, trainPct=0.9):
...
def loadAudio(seqList, datasetPath_save):  # 加载音频数据，并将其存储在一个字典中
    audioDataDic = {}
    for sequence in seqList:  # 打开对应序列的音频数据文件
        ...
        audioDataDic.update(data)  # 将加载的数据添加到一个字典
        print(f'load audio data for {sequence}')
    return audioDataDic

def loadGCC(sequence, datasetPath_save):  # 加载GCC数据
    pkl_file = open(f'{datasetPath_save}/GCC/{sequence}_GCC.pkl', 'rb')
    data = pickle.load(pkl_file)
    print(f'load GCC data for {sequence}')
    return data

def loadaudioDATA(sequence, datasetPath_save):  # 加载音频数据
...
def read_image(img_file, cvt_code=cv2.COLOR_BGR2RGB):
    ...
    if cvt_code is not None:
        img = cv2.cvtColor(img, cvt_code)  # BGR颜色空间转换为RGB颜色空间
    return img

def init_weights(model, gain=1):  # 用于初始化神经网络权重的函数，主要用于设置卷积层、批归一化层和全连接层的参数初始化方式
...
def crop_and_resize(img, center, size, out_size,  # 和视觉有关
                    border_value,
...
# def showRecimg(img, box): # 显示单个speaker的box
#     plt.imshow(img)
#     currentAxis = plt.gca()
...
def showRecimg(img, boxes): # 显示多个speaker的boxes
    """
    在图片上绘制多个矩形框
    :param img: 输入图片
    :param boxes: 边框列表，每个元素为 [x, y, width, height]
    """
    plt.imshow(img)
    currentAxis = plt.gca()
    for box in boxes:
        # 每个 box 是 [x, y, width, height]
        rect = patches.Rectangle((box[0], box[1]), box[2], box[3],linewidth=3, edgecolor='r', facecolor='none')
        currentAxis.add_patch(rect)
    # plt.savefig("test.png")  # 如果需要保存图片，取消注释
    plt.show()
...
def gcc_phat(sig, refsig, fs, max_tau=None, interp=16):
    ...
    '''
    # make sure the length for the FFT is larger or equal than len(sig) + len(refsig) 计算所需的 FFT（快速傅里叶变换）的长度
    n = sig.shape[0] + refsig.shape[0]
    # Generalized Cross Correlation Phase Transform 分别计算输入信号 (sig) 和参考信号 (refsig) 的 FFT（实值快速傅里叶变换）FFT 的长度为 n
    SIG = np.fft.rfft(sig, n=n)
    REFSIG = np.fft.rfft(refsig, n=n)
    R = SIG * np.conj(REFSIG)  # 计算输入信号和参考信号的 FFT 的交叉相关
    cc = np.fft.irfft(R / np.abs(R), n=(interp * n))  # 计算交叉相关结果的反向 FFT（快速傅里叶变换） 并将其归一化为其绝对值保存到cc中
    max_shift = int(interp * n / 2)  # 根据插值因子 (interp) 和 FFT 的长度计算出交叉相关函数中的最大可能移位
    if max_tau:
        max_shift = np.minimum(int(interp * fs * max_tau), max_shift)  # 根据最大允许时延 (max_tau)和频率 调整最大移位
    cc = np.concatenate((cc[-max_shift:], cc[:max_shift+1]))  # 将交叉相关函数 cc 围绕零延迟进行包装，从而使负移位对应于向左移动，正移位对应于向右移动
    return cc, max_shift
