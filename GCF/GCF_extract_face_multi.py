...
class getGCF(object):
    ...
    def GCFextract(self, DATA, img, GCC, fa, cam_number, faces):
        img_size = img.shape
        reGCFmaps = []
        max_d_inds = []
        ...
        for face in faces:
            ...
            reGCFmap = np.zeros(shape = (top_num, int(img_size[1]),int(img_size[0])))
            for i in range(top_num):
                reGCFmap[i] = cv2.resize(reGCFmaporg[i],(int(img_size[0]),int(img_size[1])))#resize to spl_box shape

            reGCFmaps.append(reGCFmaporg)
            max_d_inds.append(max_d_ind)

        return reGCFmaps, max_d_inds

    def au_observ(self, DATA, img, GCC, cam_number, frameNum, boxes):
        #boxes are in img coordinate
        faces = boxes
        fr = frameNum
        fa = int(2*fr-2)
        gcfmaps = self.GCFextract(DATA, img, GCC, fa, cam_number, faces)
        return gcfmaps


def cal_tau(DATA, sample2d, sample3d):  # 对所有的 3D 采样点计算时延差矩阵 tau3d
    pairNum = (1+len(DATA.audio)-1)*(len(DATA.audio)-1)/2
    tau3d = np.zeros((DATA.cfgGCF.map_num, int(pairNum), sample2d.shape[0]))
    interp = 1
    max_shift = int(interp * DATA.audio[0].shape[1])
...
def cal_rGCFmax(DATA, sample2d, tau3d, GCC, outlier,fa):  # 对输入的时延差 tau3d 和 GCC 数据，计算时间-深度对应的广义互相关值。
    m3 = 15
...
def cal_GCF2_v3(top_num, sample2d, tau3d, max_t_ind, fa, GCC, outlier, grid_x, pairNum): # 计算候选点（2D 和 3D）的广义互相关值分布图。
