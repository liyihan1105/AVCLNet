...
class YoloDetector:
    def __init__(self, weights_name, config_name, device='cuda:0', min_face=50,target_size=None, frontal=False):
        """
    ...
    def predict(self, imgs, conf_thres=0.3, iou_thres=0.5):
        ...
        if type(imgs) != list:
            ...
            if len(shapes) != 1:
                one_by_one = True
                warnings.warn(
                    f"Can't use batch predict due to different shapes of input images. Using one by one strategy.")
