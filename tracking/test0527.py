...
from models.KDNet_model0529 import KDNet
...
log_dir = os.path.abspath(os.path.join(BASE_DIR, 'log6', 'model_kdnet_ep2.pth'))
# ============================ load network============================
...
for sequence in seqList:
    ...
    for cam_number in range(3, 4):#(1, 4)1-index
...
#3d loc
            ...
            cv2.rectangle(img_viz, (int(gt2d[0] - img_anno[i][2] / 2), int(gt2d[1] - img_anno[i][3] / 2)),
                          (int(gt2d[0] + img_anno[i][2] / 2), int(gt2d[1] + img_anno[i][3] / 2)), (0, 255, 0),2)  # 绿色矩形表示真实位置
            cv2.rectangle(img_viz, (int(loc2d[0] - img_anno[i][2] / 2), int(loc2d[1] - img_anno[i][3] / 2)),
                          (int(loc2d[0] + img_anno[i][2] / 2), int(loc2d[1] + img_anno[i][3] / 2)), (0, 0, 255), 2)  # 红色矩形表示预测位置
            # 显示图像
        ...
        for i in range(len(total_result)):
            ...
            print('[{} MAE:{:.4f}]'.format(seq_result['seq'], seq_result['MAE']))
